...
class KalamSplitManager @Inject constructor(
    ...
    private val splitStatus = MutableLiveData<SplitStatus>(SplitStatus.Start)
    ...
    private fun startSplitting() {
        previewKalamProgress.value = 0
        setSplitStatus(SplitStatus.InProgress)
        kalam?.let {
    ...
    private fun reset() {
        splitStatus.value = SplitStatus.Start
        splitStart.value = 0
        splitEnd.value = 0
    ...
    override fun onEvent(event: Event) {
        ...
        when (event) {
            ...
            is KalamSplitManagerEvents.StartSplitting -> startSplitting()
